...
const EXPLANATION_ERROR_PREFIX = 'Explanation error: ';
...
async function loadConfig() {
  ...
  if (!cachedConfigPromise) {
    cachedConfigPromise = chrome.storage.local
      .get(['groqApiKey', 'translationSettings'])
      .then((config) => {
        cachedConfig = config;
        return cachedConfig;
      })
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  ...
  if (changes.groqApiKey || changes.translationSettings) {
    resetCachedConfig();
  }
});
  ...
  if (request.action === 'translateText') {
    translateText(request.text)
      ...
      .catch((error) => {
        ...
        sendResponse(`${TRANSLATION_ERROR_PREFIX}${error.message}`);
      });
  ...
  if (request.action === 'explainText') {
    explainText(request.text)
      ...
      .catch((error) => {
        ...
        sendResponse(`${EXPLANATION_ERROR_PREFIX}${error.message}`);
      });
    return true; // Required for async sendResponse
  }
...
function buildTranslationCacheKey(text, style, level) {
  return `${style}::${level}::${text}`;
}

function buildExplanationCacheKey(text, style, level) {
  return `explain::${style}::${level}::${text}`;
}

async function callGroqChatCompletion(apiKey, systemPrompt, userContent, temperature = 0.7, maxTokens = 1000) {
  ...
  try {
    ...
    if (rawBody) {
      try {
        ...
        console.warn('Failed to parse Groq response JSON', parseError);
      }
    }

    if (!response.ok) {
      const apiErrorPrefix = chrome.i18n.getMessage('apiError') || 'Groq API error';
      const detail =
        payload?.error?.message || payload?.message || rawBody || String(response.status);
      throw new Error(`${apiErrorPrefix}: ${detail}`);
    }

    const content = payload?.choices?.[0]?.message?.content?.trim();
    if (!content) {
      const fallback = chrome.i18n.getMessage('translationFailed') || 'Translation failed';
...
async function translateText(rawText) {
  ...
  const style = config?.translationSettings?.style || 'hinglish';
  const level = config?.translationSettings?.level || 'balanced';
  const cacheKey = buildTranslationCacheKey(text, style, level);
  ...
  const translatedText = await callGroqChatCompletion(apiKey, prompt, text);
...
async function explainText(rawText) {
  const text = (rawText || '').trim();
  if (!text) {
    return '';
  }
  ...
  const style = config?.translationSettings?.style || 'hinglish';
  const level = config?.translationSettings?.level || 'balanced';
  const cacheKey = buildExplanationCacheKey(text, style, level);
...
Only respond with the explanation.`;

  const explanation = await callGroqChatCompletion(apiKey, prompt, text);
